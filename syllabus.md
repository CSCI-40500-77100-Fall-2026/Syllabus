...
1. Working tests invoked via continuous integration (CI). A failing test must fail the CI run. A test that reports a failure in its output but still exits successfully does not satisfy this requirement; a testing framework handles this automatically, but a hand-rolled test must exit with a non-zero status of its own accord. A red build is not a penalty in itself, but one left red from one week to the next is, as is a green build that runs no tests.
...
- Students are responsible for any course email the instructor sends to the address in their CUNYfirst record.
